...
// };
// // Step 1 — Interview start karo + AI questions generate karo
...
// // Step 2 — Ek question ka answer submit karo
...
//         const qna = interview.qna[questionIndex];
//         if (!qna) {
//             return res.status(400).json({ message: 'Invalid question index' });
//         }

//         // AI se score + feedback lo
//         const completion = await getGroqClient().chat.completions.create({
//             model: 'llama-3.3-70b-versatile',
//             messages: [
//                 {
...
//         // Answer + score save karo
//         interview.qna[questionIndex].answer = answer;
//         interview.qna[questionIndex].aiScore = evaluation.score;
//         interview.qna[questionIndex].aiFeedback = evaluation.feedback;

//         // Agar sab questions answer ho gaye toh complete karo
...
exports.submitAnswer = async (req, res) => {
    ...
    try {
        ...
        if (!finalAnswer) {
            return res.status(400).json({ message: 'Answer is required — text, audio, or video' });
        }
        if (!finalAnswer || finalAnswer.trim().length < 5) {
            return res.status(400).json({
                message: 'Audio mein koi speech nahi mili — dobara record karo'
            });
        }
